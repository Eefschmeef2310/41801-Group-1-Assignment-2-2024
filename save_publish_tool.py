...
#Setting up plugins for exporting
if not cmds.pluginInfo("fbxmaya", query=True, loaded=True):
    cmds.loadPlugin("fbxmaya")
if not cmds.pluginInfo("mayaUsdPlugin", query=True, loaded=True):
    cmds.loadPlugin("mayaUsdPlugin")

#Get latest version of the file you are working on in the WIP directory
...
#Gets called by publish_button() and when save button is pressed
def save_file():
...
#Gets called by pressing publish button
def publish_button():
    cmds.text(feedback, e=True, label ="Publishing...")
    pub_dir = publish_path + re.sub(wip_path, "", re.sub(publish_path, "", directory))

    #The current file is a shot file from sequence folder .
    #So we have to loop through the character and prop groups and export each individual asset
    ...
    #Else means that it's just an asset file
    else:
        export_file(pub_dir, file_no_end)

    #After exporting files, save a copy in publish folder then save
    publish_copy(pub_dir)
    save_file()

#Function for exporting each asset in a shot
def export_asset(pub_dir, name):
    ...
    #Selection is done before calling this function
...
#Function for exporting the whole file not each asset
...
#Creates a copy of file in the source folder in the publish directory
def publish_copy(pub_dir):
    copy = pub_dir + "/" + base_file
    cmds.file(rename=copy)
    cmds.file(save=True, type="mayaBinary")

#Creating Window and UI stuff
